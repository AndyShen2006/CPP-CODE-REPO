...
public:
    ...
    inline static Matrix<Type> NaiveMultiply(const Matrix<Type>& A, const Matrix<Type>& B)
    {
...
public:
    ...
    inline Matrix<Type> operator*(const Matrix<Type>& rhs) const
    {
        if (this->_column != rhs.row()) {
            ...
            int align = 0; // std::ceil(std::log2(std::max({ this->row(), this->column(), rhs.row(), rhs.column() })));
            if (true || std::pow(align, 2.73) * 114514 >= this->row() * this->column() * rhs.column()) {
    ...
    inline void quickPow(Matrix<Type>& Ans, size_t x) const
    {
        if (x > 0) {
            auto a(*this);
            while (x) {
...
struct MyInt {
    ...
    inline MyInt operator+(const MyInt& rhs) const
    {
        return (data + rhs.data) % MOD;
    }
    inline MyInt operator-(const MyInt& rhs) const
    {
        return (data - rhs.data) % MOD;
    ...
    inline operator int() const // NOLINT
    {
        return data;
    }
};
